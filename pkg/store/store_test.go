...
//			},
//		}
//		deleteRes, err := st.(*store).likesCollection.DeleteOne(context.TODO(), filter)
//		assert.NoError(t, err)
...
//}
//
////func Test_store_PutSeen(t *testing.T) {
////	collection, _ := prepareCollection(seenCollectionName)
////	st := NewStore(nil, collection, nil)
////	t.Run(putNewLike, func(t *testing.T) {
////		err := st.PutSeen(whoID, whomeID)
////		assert.NoError(t ,err)
////		likes, err := st.GetSeen(whoID)
////		assert.NoError(t, err)
////		assert.Equal(t, int64(whoID), likes.Who)
////		filter := bson.D{
////			{
...
////			},
////		}
////		deleteRes, err := st.(*store).seenCollection.DeleteOne(context.TODO(), filter)
////		assert.NoError(t, err)
////		assert.Equal(t, int64(1), deleteRes.DeletedCount)
////	})
////
...
////		err := st.PutSeen(whoID, whomeID)
////		assert.NoError(t ,err)
////		err = st.PutSeen(whoID, anotherWhome)
////		assert.NoError(t, err)
////		likes, err := st.GetSeen(whoID)
////		assert.NoError(t, err)
////		assert.Equal(t, int64(whoID), likes.Who)
